py::enum_<MsgType_ToDevice>(m, "MsgType_ToDevice")
    ...
    .value("UpdatePidConfig", MsgType_ToDevice::UpdatePidConfig)
    .value("RequestAngle", MsgType_ToDevice::RequestAngle)
    .value("SetAngle", MsgType_ToDevice::SetAngle)
    .value("RequestAllCalibrationData", MsgType_ToDevice::RequestAllCalibrationData)
...
py::enum_<MsgType_ToHost>(m, "MsgType_ToHost")
    ...
    .value("Reserved2", MsgType_ToHost::Reserved2)
    .value("Angle", MsgType_ToHost::Angle)
    ...
    .value("FaultStatusData", MsgType_ToHost::FaultStatusData)
    .value("NewFault", MsgType_ToHost::NewFault)
    ...
    .value("FramingError", MsgType_ToHost::FramingError)
    .value("Unknown", MsgType_ToHost::Unknown)
    .value("NoMessage", MsgType_ToHost::NoMessage)
    ;
...
py::enum_<error_reason_t>(m, "error_reason_t")
    ...
    .value("E_FAULT_GENERATED", error_reason_t::E_FAULT_GENERATED)
    .value("E_NAK_RECEIVED", error_reason_t::E_NAK_RECEIVED)
    .value("E_ACK_RECEIVED", error_reason_t::E_ACK_RECEIVED)
    .value("E_NOT_IMPLEMENTED", error_reason_t::E_NOT_IMPLEMENTED)
    ...
    .value("E_FIRMWARE_UPDATE_ALREADY_IN_PROGRESS", error_reason_t::E_FIRMWARE_UPDATE_ALREADY_IN_PROGRESS)
    ...
    .value("E_FLASH_OPEN_FAILED", error_reason_t::E_FLASH_OPEN_FAILED)
    ...
    .value("E_PROTOCOL_VERSION_MISMATCH", error_reason_t::E_PROTOCOL_VERSION_MISMATCH)
    ...
    .value("E_I2C0_INIT_FAILED", error_reason_t::E_I2C0_INIT_FAILED)
    .value("E_I2C1_INIT_FAILED", error_reason_t::E_I2C1_INIT_FAILED)
    .value("E_LOADCELL_NOT_IN_CALIBRATION_MODE", error_reason_t::E_LOADCELL_NOT_IN_CALIBRATION_MODE)
    ...
    .value("E_ACCELEROMETER_FIRST_CALIBRATION_POINT_EXPIRED", error_reason_t::E_ACCELEROMETER_FIRST_CALIBRATION_POINT_EXPIRED)
    ...
    .value("E_ANGLE_OUT_OF_RANGE", error_reason_t::E_ANGLE_OUT_OF_RANGE)
    ...
    .value("E_UART_OVERFLOW", error_reason_t::E_UART_OVERFLOW)
    .value("E_UNKNOWN", error_reason_t::E_UNKNOWN)
...
py::enum_<status_bits_t>(m, "status_bits_t")
    ...
    .value("ORIENTATION_HAS_CALIBRATION_BIT", status_bits_t::ORIENTATION_HAS_CALIBRATION_BIT)
...
py::class_<version_t>(m, "Version")
    .def(py::init<>())
    .def_readwrite("major", &version_t::major)
    .def_readwrite("minor", &version_t::minor)
    .def_readwrite("patch", &version_t::patch)
    .def("serialize", [](version_t& s) { return py::bytes(reinterpret_cast<const char*>(&s), sizeof(s)); })
    .def("sizeof", [](version_t& s) { return sizeof(s); })
    .def_static("deserialize", [](py::bytes b) { std::string s = b; if (s.size() != sizeof(version_t)) throw std::runtime_error("deserialize: wrong size"); version_t result; std::memcpy(&result, s.data(), sizeof(version_t)); return result; })
    ;
...
py::class_<error_reason_payload_t>(m, "ErrorReasonPayload")
    .def(py::init<>())
    .def_readwrite("reason", &error_reason_payload_t::reason)
    .def("serialize", [](error_reason_payload_t& s) { return py::bytes(reinterpret_cast<const char*>(&s), sizeof(s)); })
    .def("sizeof", [](error_reason_payload_t& s) { return sizeof(s); })
    .def_static("deserialize", [](py::bytes b) { std::string s = b; if (s.size() != sizeof(error_reason_payload_t)) throw std::runtime_error("deserialize: wrong size"); error_reason_payload_t result; std::memcpy(&result, s.data(), sizeof(error_reason_payload_t)); return result; })
    ;

py::class_<ack_payload_t>(m, "AckPayload")
    .def(py::init<>())
    .def_readwrite("msg_type", &ack_payload_t::msg_type)
    .def("serialize", [](ack_payload_t& s) { return py::bytes(reinterpret_cast<const char*>(&s), sizeof(s)); })
    .def("sizeof", [](ack_payload_t& s) { return sizeof(s); })
    .def_static("deserialize", [](py::bytes b) { std::string s = b; if (s.size() != sizeof(ack_payload_t)) throw std::runtime_error("deserialize: wrong size"); ack_payload_t result; std::memcpy(&result, s.data(), sizeof(ack_payload_t)); return result; })
    ;

py::class_<fault_payload_t>(m, "FaultPayload")
    .def(py::init<>())
    .def_readwrite("reason", &fault_payload_t::reason)
    .def("serialize", [](fault_payload_t& s) { return py::bytes(reinterpret_cast<const char*>(&s), sizeof(s)); })
    .def("sizeof", [](fault_payload_t& s) { return sizeof(s); })
    .def_static("deserialize", [](py::bytes b) { std::string s = b; if (s.size() != sizeof(fault_payload_t)) throw std::runtime_error("deserialize: wrong size"); fault_payload_t result; std::memcpy(&result, s.data(), sizeof(fault_payload_t)); return result; })
    ;

py::class_<fault_status_payload_t>(m, "FaultStatusPayload")
    .def(py::init<>())
    .def_readwrite("fault_status", &fault_status_payload_t::fault_status)
    .def("serialize", [](fault_status_payload_t& s) { return py::bytes(reinterpret_cast<const char*>(&s), sizeof(s)); })
    .def("sizeof", [](fault_status_payload_t& s) { return sizeof(s); })
    .def_static("deserialize", [](py::bytes b) { std::string s = b; if (s.size() != sizeof(fault_status_payload_t)) throw std::runtime_error("deserialize: wrong size"); fault_status_payload_t result; std::memcpy(&result, s.data(), sizeof(fault_status_payload_t)); return result; })
    ;
...
py::class_<handshake_to_host_payload_t>(m, "HandshakeToHostPayload")
    ...
    .def_readwrite("firmware", &handshake_to_host_payload_t::firmware)
